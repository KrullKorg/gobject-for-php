...
if test "$PHP_GOBJECT" != "no"; then
  ...
  PHP_NEW_EXTENSION(gobject, \
    gobject_ext.c gobject_girepository.c gobject_helpers.c gobject_paramspec_class.c gobject_type_class.c gobject_gobject_class.c gobject_closure.c gobject_signal_class.c \
    , $ext_shared)
  ...
  if test -f "$PKG_CONFIG"; then
    ...
    if $PKG_CONFIG --exists gobject-2.0; then
        gobject_version_full=`$PKG_CONFIG --modversion gobject-2.0 gobject-introspection-1.0`
        AC_MSG_RESULT([found $gobject_version_full])
        GOBJECT_LIBS="$LDFLAGS `$PKG_CONFIG --libs gobject-2.0 gobject-introspection-1.0`"
        GOBJECT_INCS="$CFLAGS `$PKG_CONFIG --cflags-only-I gobject-2.0 gobject-introspection-1.0`"
